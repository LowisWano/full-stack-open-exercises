...
sequenceDiagram
    ...
    browser->>server: GET requests https://studies.cs.helsinki.fi/exampleapp/main.css
    activate server
    ...
    deactivate server
    Note right of browser: browser renders the notes using the `redrawNotes()` function
```